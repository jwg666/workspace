 ...
 * @author Joram Barrez
 */
public class HroisProcessDiagramCanvas {

  protected static final Logger LOGGER = LoggerFactory.getLogger(HroisProcessDiagramCanvas.class);
  ...
  protected static Color FINSHED_HIGHLIGHT_COLOR = Color.GREEN;
  protected static Color LABEL_COLOR = new Color(0, 0, 255);
   ...
   */
  public HroisProcessDiagramCanvas(int width, int height) {
   ...
   *          below minX on the horizontal scale will be cropped.
   */
  public HroisProcessDiagramCanvas(int width, int height, int minX, int minY) {
    this(width, height);
    this.minX = minX;
    this.minY = minY;
  }
  ...
  public void drawCollapsedSubProcess(String name, int x, int y, int width, int height, boolean thickBorder) {
   // drawCollapsedTask(name, x, y, width, height, false);
		    ...
		    // text
		    if (name != null && name.length() > 0) {
		    	drawSubMultilineText(name, x, y, width, height);
		    }
  }
  
  protected void drawSubMultilineText(String text, int x, int y, int boxWidth, int boxHeight) {
	    int availableHeight = boxHeight - ICON_SIZE - ICON_PADDING;
	    
	    // Create an attributed string based in input text
	    AttributedString attributedString = new AttributedString(text);
	    attributedString.addAttribute(TextAttribute.FONT, new Font("黑体", Font.BOLD, 20));
	    attributedString.addAttribute(TextAttribute.FOREGROUND, Color.black);
	    //attributedString.addAttribute(TextAttribute.FOREGROUND, Color.black);
  ...
  public void drawFinshedHighLight(int x, int y, int width, int height) {
	  Paint originalPaint = g.getPaint();
	  Stroke originalStroke = g.getStroke();
	  
	  g.setPaint(FINSHED_HIGHLIGHT_COLOR);
	  g.setStroke(THICK_TASK_BORDER_STROKE);
