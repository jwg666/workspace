package com.neusoft.activiti.diagram;
...
/**
 * 重构activiti的流程图生成代码，显示已完成节点
 * @author 秦焰培
 *
 */
public class HroisProcessDiagramGenerator  {
	...
	protected static final Map<Class<? extends BaseElement>, ActivityDrawInstruction> activityDrawInstructions = new HashMap<Class<? extends BaseElement>, ActivityDrawInstruction>();
	  ...
	  static {
	    ...
	    activityDrawInstructions.put(StartEvent.class, new ActivityDrawInstruction() {
	      ...
	      public void draw(HroisProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel, FlowNode flowNode) {
	  ...
	  public static InputStream generatePngDiagram(BpmnModel bpmnModel) {
	    return generateDiagram(bpmnModel, "png", Collections.<String> emptySet(), Collections.<String> emptySet(), Collections.<String> emptySet());
	  }
	  ...
	  public static InputStream generateJpgDiagram(BpmnModel bpmnModel) {
	    return generateDiagram(bpmnModel, "jpg", Collections.<String> emptySet(), Collections.<String> emptySet(), Collections.<String> emptySet());
	  }

	  protected static HroisProcessDiagramCanvas generateDiagram(BpmnModel bpmnModel, Set<String> highLightedActivities) {
		  return generateDiagram(bpmnModel, highLightedActivities, Collections.<String> emptySet(),Collections.<String> emptySet());
	  }
	  
	  protected static HroisProcessDiagramCanvas generateDiagram(BpmnModel bpmnModel, Set<String> highLightedActivities,Set<String> highLightedFlows,Set<String> finshedActivityIds) {
	    HroisProcessDiagramCanvas processDiagramCanvas = initHroisProcessDiagramCanvas(bpmnModel);
//	    // Draw pool shape, if process is participant in collaboration
	    ...
	    for (FlowNode flowNode : bpmnModel.getProcesses().get(0).findFlowElementsOfType(FlowNode.class)) {
	      drawActivity(processDiagramCanvas, bpmnModel, flowNode, highLightedActivities, highLightedFlows, finshedActivityIds);
	    }
	    return processDiagramCanvas;
	  }
	  
	  public static InputStream generateDiagram(BpmnModel bpmnModel, String imageType, Set<String> highLightedActivities,Set<String> highLightedFlows,Set<String> finshedActivityIds) {
	    HroisProcessDiagramCanvas canvas = generateDiagram(bpmnModel, highLightedActivities, highLightedFlows,finshedActivityIds);
	 ...
	 /* public static InputStream generateDiagram(BpmnModel bpmnModel, String imageType, List<String> highLightedActivities, List<String> highLightedFlows) {
	    return generateDiagram(bpmnModel, highLightedActivities, highLightedFlows,Collections.<String> emptyList()).generateImage(imageType);
	  }*/

	  protected static void drawActivity(HroisProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel, FlowNode flowNode, Set<String> highLightedActivities, Set<String> highLightedFlows,Set<String> finshedActivityIds) {
	    ...
	    if (drawInstruction != null) {
	      ...
	      //Darw finished activities
	      if (finshedActivityIds.contains(flowNode.getId())) {
		        drawFinshedHighLight(processDiagramCanvas, bpmnModel.getGraphicInfo(flowNode.getId()));
		   }
...
//	    }
	    ...
	    if (flowNode instanceof FlowElementsContainer) {
	      for (FlowElement nestedFlowElement : ((FlowElementsContainer) flowNode).getFlowElements()) {
	        if (nestedFlowElement instanceof FlowNode) {
	          drawActivity(processDiagramCanvas, bpmnModel, (FlowNode) nestedFlowElement, highLightedActivities, highLightedFlows, finshedActivityIds);
	        }
	      }
	    }
	  }

	  private static void drawHighLight(HroisProcessDiagramCanvas processDiagramCanvas, GraphicInfo graphicInfo) {
	    processDiagramCanvas.drawHighLight((int) graphicInfo.getX(), (int) graphicInfo.getY(), (int) graphicInfo.getWidth(), (int) graphicInfo.getHeight());

	  }
	  private static void drawFinshedHighLight(HroisProcessDiagramCanvas processDiagramCanvas, GraphicInfo graphicInfo) {
		  processDiagramCanvas.drawFinshedHighLight((int) graphicInfo.getX(), (int) graphicInfo.getY(), (int) graphicInfo.getWidth(), (int) graphicInfo.getHeight());
	  }

	  protected static HroisProcessDiagramCanvas initHroisProcessDiagramCanvas(BpmnModel bpmnModel) {
	    ...
	    return new HroisProcessDiagramCanvas((int) maxX + 10,(int) maxY + 10, (int) minX, (int) minY);
	  }
	  ...
	  protected interface ActivityDrawInstruction {
	    ...
	    void draw(HroisProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel, FlowNode flowNode);
