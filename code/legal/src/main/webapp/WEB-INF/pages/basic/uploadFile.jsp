...
<script type="text/javascript" charset="utf-8">
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			url : '${dynamicURL}/basic/uploadFileAction/searchUploadFile.do',
			title : '文件列表',
			iconCls : 'icon-save',
			pagination : true,
			pagePosition : 'bottom',
				...
				{field:'download',title:'预览',align:'left',width:40,
					formatter:function(value,row,index){
						if(row.status!="1"){
							...
							return '<a href="${dynamicURL}/basic/uploadFileAction/downloadImage.do?fileId=' + row.id + '" target="_blank"  ><img style="height:20px;" src="${dynamicURL}/basic/uploadFileAction/downloadImage.do?fileId='+ row.id +  '"/></a>';
						}else{
							return '<a href="${dynamicURL}/basic/fileUploadAction/downloadFile.do?fileId='+row.id+'" target="_blank" >下载 </a>';
				...
				{field:'fileName',title:'文件名',align:'center',width:200,
					formatter:function(value,row,index){
						if(row.contentType != null && row.contentType.indexOf("image")>-1){
							return '<a href="${dynamicURL}/basic/uploadFileAction/downloadImage.do?fileId=' + row.id + '" target="_blank"  >' + row.fileName + '</a>';
						}else{
							return row.fileName;
						}
					}
		...
		upLoadForm = $('#upLoadForm').form({
			url : '${dynamicURL}/basic/uploadFileAction/uplaodFile.do',
		...
		upLoadLocalForm = $('#upLoadLocalForm').form({
			url : '${dynamicURL}/basic/uploadFileAction/uplaodFileToLocal.do',
			success:function(data) {
				var json = $.parseJSON(data);
				var obj = json.obj;
				if (json && json.success) {
		...
		updateForm = $('#updateFileForm').form({
			url : '${dynamicURL}/basic/uploadFileAction/updateFile.do',
			success:function(data) {
				var json = $.parseJSON(data);
				var obj = json.obj;
				if (json && json.success) {
		...
		if (rows.length > 0) {
			$.messager.confirm('请确认', '您要删除当前文件？', function(r) {
				if (r) {
					...
					$.ajax({
						url : '${dynamicURL}/basic/uploadFileAction/deleteFile.do',
