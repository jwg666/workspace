...
<script type="text/javascript" charset="utf-8">
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			url : 'moneyRateAction!datagrid.action',
			title : '汇率列表',
			iconCls : 'icon-save',
			pagination : true,
			pagePosition : 'bottom',
			...
			columns : [ [ 
			   ...
			   {field:'moneyType',title:'币种',align:'center',sortable:false,
			  	    editor : {
							type : 'validatebox',
							options : {}
					},
			   ...
			   {field:'toUsdRate',title:'兑美元汇率',align:'center',sortable:false,
			  	    editor : {
							type : 'validatebox',
							options : {}
					},
					formatter:function(value,row,index){
						return row.toUsdRate;
					}
				},				
			   {field:'toRmbRate',title:'兑人民币汇率',align:'center',sortable:false,
			  	    editor : {
							type : 'validatebox',
							options : {}
					},
					formatter:function(value,row,index){
						return row.toRmbRate;
					}
				},				
			   {field:'startDate',title:'有效开始',align:'center',sortable:false,
			  	    editor : {
							type : 'datebox',
							options : {}
					},
			   ...
			   {field:'endDate',title:'有效结束',align:'center',sortable:false,
			  	    editor : {
							type : 'datebox',
							options : {}
					},
			   ...
			   {field:'activeFlag',title:'有效标识',align:'center',sortable:false,
			  	    editor : {
			  	    	type:'combobox',
						editable: false,
						options:{
			...
			onAfterEdit : function(rowIndex, rowData, changes) {
				...
				if (inserted.length > 0) {
					url = 'moneyRateAction!add.action';
				}
				if (updated.length > 0) {
					url = 'moneyRateAction!edit.action';
				}
		...
		if (rows.length > 0) {
			$.messager.confirm('请确认', '您要删除当前所选项目？', function(r) {
				if (r) {
					...
					$.ajax({
						url : 'moneyRateAction!delete.action',
						data : {
							ids : ids.join(',')
						},
						dataType : 'json',
			...
			<div class="part_zoc">
				...
				<div class="item25">
					<div class="itemleft60">币种：</div>
						<div class="righttext">
							<input id="moneyType" name="moneyType" type="text"
								style="width: 125px" />
						</div>
					</div>
					<div class="item25">
						<div class="itemleft60">有效开始：</div>
