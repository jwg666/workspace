			...
			<div style="width: 100%;">
				...
				<s:form action="searchAdminInGroup" namespace="/security"
					...
					<table class="form_table">
						<tr>
							<th nowrap="nowrap">员工号：</th>
							<td><s:hidden name="group.id" id="thisgroupid" /> <s:textfield
									name="userGroup.User.empCode" /></td>
							<th>用戶名：</th>
							<td><s:textfield name="userGroup.user.name" /></td>
						</tr>
						<tr>
							<th>Email：</th>
							<td><s:textfield name="userGroup.user.email" /></td>
				...
				<table id="rounded-corner" class="color_table" cellspacing="0"
					...
					<thead>
						<tr>
							...
							<th style="width: 100px; font-size: 12px;">员工号</th>
					...
					<tbody>
						<!-- 数据行 -->
						<s:iterator value="pagerUserGroup.records" var="user"
							status="status">
							<tr>
								<td style="width: 12PX;"><input name="usercheck"
									type="checkbox" id="usercheck<s:property value='user.id'/>"
				...
				<div id="footer-container" class="dd-fd"
					...
					<p:pagination pager="pagerUserGroup" formId="searchAdminForm"></p:pagination>
				</div>
			</div>
		</div>
	</dd>
