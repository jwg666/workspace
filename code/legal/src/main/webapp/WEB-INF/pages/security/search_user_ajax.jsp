 ...
 <s:form method="get" id="searchUserForm" onsubmit="return false;">
	<table class="form_table">
		<tr>
			<th>员工号：</th>
			<td>
			<s:textfield name="user.empCode" id="empCode"/></td>
			<th>用戶名：</th>
			<td>
...
$("#sbm").click(function(){
	...
	for(var i = 0; i < ids.length; i++) {
		...
		alert(rowdata.email);
		var input1 = "<input type='hidden' name='myusers["+i+"].empCode' value='"+rowdata.empCode+"'/>";
		frm.append(input1);
		
		var input2 = "<input type='hidden' name='myusers["+i+"].name' value='"+rowdata.name+"'/>";
		frm.append(input2);
	...
	alert('ssss');
 	...
 	$(document).ready(function(){ 		
		...
		$("#list1").jqGrid({
			...
			colModel: [
				{name:'empCode',index:'empCode', frozen:true, align:"center", editable:false},
