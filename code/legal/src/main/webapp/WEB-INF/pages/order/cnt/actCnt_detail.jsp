...
<script type="text/javascript">
	...
	$(function(){
		datagrid_contract_one = $('#datagrid_contract_one').datagrid({
			url : "${dynamicURL}/salesOrder/salesOrderAction!getActCntDetailPara.do?para="+para,
			//pagination : true,
			//pagePosition : 'bottom',
			rownumbers : true,
			pageSize : 10,
...
// 			    	var bb = $('.datagrid-view2').find('.datagrid-footer').find("[field='nowBudgetQuatity']").find('div').text();
			    	var rdd = datagrid_contract_two.datagrid('getSelected');
			    	...
			    	if(rowss.length > 1){
						...
						$("[field='nowBudgetQuatity']").find("input").keyup(function(){
							...
							for(var kk = 0; kk < rowss.length; kk++){
								sumnum = parseInt(rowss[kk].nowBudgetQuatity) + sumnum;
							}
							var rrows = datagrid_contract_two.datagrid('getFooterRows');
							var numss = sumnum - parseInt(rdd.nowBudgetQuatity) + parseInt(nowBudgetQuatity);
							$('#datagrid_contract_two').datagrid('reloadFooter',[{budgetOrderQuatity: '总计:' , nowBudgetQuatity : numss}]);
							...
							if(parseInt(nowBudgetQuatity) > parseInt(row.budgetOrderQuatity)) {
 								...
 								$("[field='nowBudgetQuatity']").find("input").val('0');
 								$('#datagrid_contract_two').datagrid('reloadFooter',[{budgetOrderQuatity: '总计:'}]);
							}else{
						...
						$("[field='nowBudgetQuatity']").find("input").keyup(function(){
							...
							var nowBudgetQuatity = $(this).val();
							//总数的变化    
							$('#datagrid_contract_two').datagrid('reloadFooter',[{budgetOrderQuatity: '总计:' , nowBudgetQuatity : nowBudgetQuatity}]);
							var row = datagrid_contract_two.datagrid('getSelected');
							var rows = datagrid_contract_one.datagrid('getRows');
							for(var k = 0; k < rows.length ; k++){
								var idx = datagrid_contract_one.datagrid('getRowIndex',rows[k]);
							...
							if(parseInt(nowBudgetQuatity)>parseInt(row.budgetOrderQuatity)) {
 								$.messager.alert('<s:text name="global.form.prompt">提示</s:text>', '<s:text name="global.prepareorder.quatityRemain">所填写数量大于可预算数量！</s:text>', 'error');
 								$("[field='nowBudgetQuatity']").find("input").val('0');
 								$('#datagrid_contract_two').datagrid('reloadFooter',[{budgetOrderQuatity: '<s:text name="global.account.total">总计</s:text>:', nowBudgetQuatity : 0}]);
							}else{
	 							var ed = datagrid_contract_two.datagrid('getEditor',{index:index,field:'nowBudgetNumber'});
	 							//根据成套系数 得到件数
	 							if(row.completeQuotiety == 0 || row.completeQuotiety == null){
	...
	function changeFromOneToTwo(){
	    ...
	    link.hidden=true;
	    var str = 100;
	    setTimeout("doEnableLink();", 1000 * 2);
		...
		if(rows.length == 0){
			$.messager.alert('<s:text name="global.form.prompt">提示</s:text>', '<s:text name="global.info.selectDateList">请选择要操作的数据！</s:text>', 'error');
			link.hidden=false;
		...
		var datass = $('#datagrid_contract_two').datagrid('getRows');
		var nums = 0;
		for(var p = 0; p < datass.length ; p++){
			nums = parseInt(datass[p].nowBudgetQuatity) + nums;
		}
		$('#datagrid_contract_two').datagrid('reloadFooter',[{budgetOrderQuatity: '总计:' , nowBudgetQuatity : nums}]);
	}
	function saveLoadResult(){
			...
			if(f =="1"){
				$.messager.alert('<s:text name="global.form.prompt">提示</s:text>', '<s:text name="global.actcnt.need.quatity">请先填写装箱相应数量</s:text>');
				$.messager.progress('close');
			}else{
			//ajax同步先生成预算批次号
			if(rows.length > 0){
...
//			plugin.pstart(val);
			...
			$.messager.alert('<s:text name="global.form.prompt">提示</s:text>', '<s:text name="global.order.hrois.newSoftware">请您去HROIS登录页面下载新版本的网页版本装箱软件!</s:text>');
		}
	}
	//以上装箱软件 调用 -----
	...
	function saveActCnt(){
		...
		if(cc == ""){
 			$.messager.progress('close');
 			$.messager.alert('<s:text name="global.form.prompt">提示</s:text>', '<s:text name="global.order.needActCntCode">先调用装箱软件生成预算批次号！</s:text>', 'error');
 		}else{
		...
		if(numT == nNum){
	 		...
	 		$.ajax({
	 			...
	 			success : function(response) {
	 				$.messager.progress('close');
	 				$.messager.alert('<s:text name="global.form.prompt">提示</s:text>', '<s:text name="global.info.saveSuccess">保存成功！</s:text>');
	 				if(indexx == "1"){
	 					parent.window.HROS.window.close(currentappid);
	 				}else{
	 					//代办刷新
			...
			$.messager.progress('close');
			$.messager.alert('<s:text name="global.form.prompt">提示</s:text>', '<s:text name="global.actcnt.error">装箱有异常,请重新确认！</s:text>', 'error');
		}
 		}
	}
	function byebye(){
