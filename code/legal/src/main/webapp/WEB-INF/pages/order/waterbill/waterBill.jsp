...
<script type="text/javascript" charset="utf-8">
			...
			onLoadSuccess : function(data) {
				...
				$('#customerCode0').combogrid({
					...
					onLoadSuccess : function() {
						$('#customerCode0').combogrid('setValue',data.customerCode);
	...
	function saveEdit() {
		//验证价格是否低于已经使用的价格
		$.ajax({
				url : 'waterBillAction!checkAmount.action',
				type : 'post',
				data : {
				...
				async:false,
				success : function(response) {
					if(response.obj>$("#amount").val()){
						$.messager.alert('提示', '修改金额低于已使用金额，不允许修改！', 'info');
						return;
					}
				}
		});
			...
			<div class="part_zoc">
					...
					<div class="item33">
						...
						<div class="righttext">
							...
							<input type="text" id="customerCode0" name="customerCode"
								data-options="required:true,editable:false" missingMessage="请填写客户编号"
								style="width: 155px;"></input>
						</div>
					</div>
				</div>
				<div class="oneline">
					...
					<div class="item33">
						<div class="itemleft">金额：</div>
						<div class="righttext">
							<input name="amount" id="amount" type="text" class="easyui-numberbox"
								required="true" data-options="precision:2"
								missingMessage="请输入金额" style="width: 155px;" />
						</div>
					</div>
