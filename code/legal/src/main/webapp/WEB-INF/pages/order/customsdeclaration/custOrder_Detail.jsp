...
$(function() {
	 ...
	 /* $.ajax({
		...
		success : function(response) {  */
			...
			datagrid = $('#datagrid').datagrid({
				url : 'custOrderAction!checkTaskBybookCode1.do?definitionKey=declarationApply&bookCode='+$('#bookCode').val()+'&mergeCustFlag='+$('#mergeCustFlagId').val(),
				title : '明细信息',
				iconCls : 'icon-save',
	 			pagination : true,
				pagePosition : 'bottom',
	...
	//}); 
		    //自定义格式校验
			$('#custNumid').validatebox({
			    validType: 'custnumvalidate["'+'请输入18位数字字母编号!'+'"]'  
			});
});

function save(){
	var custNum = $('#custNumid').val();
	var custDate =  $('#custDate').datebox('getValue');
	var bookCode = $('#bookCode').val();
	var orderCode='';
	var orderToVendor='';
        if (custNum!=null&&custNum!=''){
        	if(!(/^[0-9a-zA-Z]*$/g.test(custNum))){
        		return ;
        	}else if(custNum.length!=18){
        		return ;
        	}
        } else { 
            return ;
        }
	if(custDate==null||custDate==""||custNum==null||custNum==""){
		$.messager.alert('提示','报关单号和报关时间不可为空','');
		return false;
	}
	...
	for ( var i = 0; i < rows.length; i++) {
		if(i!=rows.length-1){
			...
			taskIds=taskIds+rows[i].taskId;
			orderCode=orderCode+rows[i].orderCode;
			orderToVendor=rows[i].orderCode;
		}
	}
	$.messager.progress({
		text : '数据加载中....',
	...
	$.ajax({
		url : 'custOrderAction!addCustOrder.do',
		data : {
			custNum : custNum,
			custDate : custDate,
			bookCode : bookCode,
			...
			<form id="searchForm">
	            ...
	            <div class="oneline">
	                ...
	                <div class="item25">
	                    ...
	                    <div class="righttext">
	                    	<input type="text" class="short80" readonly="readonly" name="custCompany"/>
	                    </div>
	                </div>
	                 <div class="item25 lastitem">
	                    <div class="itemleft60"><s:text name="order.custorder.custNum">报关单号</s:text>：</div>
	                    <div class="righttext">
	                    	<input type="text" id="custNumid" data-options="required:true"  class="easyui-validatebox short80" name="custNum"/>
	                    </div>
	                </div>
	                <div class="item25">
	                    <div class="itemleft60"><s:text name="order.custorder.custDate">报关时间</s:text>：</div>
	                    <div class="righttext">
	                    	<input name="custDate" id="custDate"  data-options="required:true" class="easyui-datebox short80" editable="false"/>
	                    </div>
	                </div>
	                <div class="item25 lastitem">
