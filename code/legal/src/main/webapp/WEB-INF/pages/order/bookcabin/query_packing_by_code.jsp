...
.formtable tr.tr td {width: 250px;height: 17px;}
.formtable tr.tr td.t-title { padding:7px 5px 7px 10px; font-size:14px;font-weight:bold; color:#FFFF; }
.formtable tr.tr td.t-content { padding:10px 12px 10px 10px; font-size:18px;font-weight:bold;}
.formtable tr.tr td.t-head { background:#f2f2f2;}
.oneLine {
...
// 			idField : 'orderCode',
			...
			toolbar:[{
				text : '<s:text name="global.form.save" >保存</s:text>',
				iconCls : 'icon-save',
				handler : function() {
					save();//操作
				}
				...
				text : '<s:text name="global.form.delete" >删除</s:text>',
				iconCls : 'icon-remove',
				handler : function() {
					deleteRow();//操作
				}
			},'-',{
				text : '<s:text name="global.info.back" >回退</s:text>',
				iconCls : 'icon-redo',
				handler : function() {
					rollBack();//操作
				}
		...
		if(orderCode1!=null&&orderCode1!=''){
			if(orderType1!=null&&orderType1!=''){
				$('#orderCodeId').val(orderCode1);
				  var sel = document.getElementById("orderType");
		            for (var i = 0; i < sel.length; i++) {
		                if (sel[i].value == orderType1) {
		                    //后台传过来的数据
		                    sel[i].selected = true;
		                }
		            }
					...
					$.messager.confirm('请确认', '您要删除当前所选项目？', function(r) {
						if (r) {
							for ( var i = 0; i < rows.length; i++) {
								if(i!=rows.length-1)
									packingListIds=packingListIds+"packingListIds="+rows[i].packingListId+"&";
								else packingListIds=packingListIds+"packingListIds="+rows[i].packingListId;
							}
							$.ajax({
	...
	function rollBack(){
		...
		if(printFlag){
			var bookCode=$("#bookCode").val();
			$.messager.confirm('请确认', '您要回退当前订单？', function(r) {
				if (r) {
					$.ajax({
	 ...
	 function checkEmp(){
		...
		function appletPrint(){
			...
			if($("#bookCode").val().indexOf("-")>=0){
				...
				window.open("http://hrois.haier.net/report/Report-Guage.do?reportId=1281c8c4-7a21-4829-b1cb-cd8531b3cb21&silent=yes&isResultant=false&book_code="+bookCode[0]+"&falg="+bookCode[1]);
			}else{
				window.open("http://hrois.haier.net/report/Report-Guage.do?reportId=1281c8c4-7a21-4829-b1cb-cd8531b3cb21&silent=yes&isResultant=false&book_code="+$("#bookCode").val()+"&falg=01");
			}
		}
</script>
...
<body class="easyui-layout zoc">
		...
		<form id="searchForm">
			...
			<div class="oneline">
				<div class="item25">
					...
					<div class="righttext">
						<input name="bookCode" id="bookCode" readonly="readonly" type="text" value="${bookCode}"/>
					</div>
				</div>
				<div class="item33">
	...
	<div region="center" id="printBody" border="false" style="overflow: auto;" class="zoc"
		align="center">
		...
		<div class="part_zoc zoc" style="width: 1000px;">
			...
			<form id="searchPackForm" style="height: 100%; width: 1000px;">
				<table width="100%" class="formtable" style="height: 100%;">
					<tr class="tr">
						<td colspan="4" class="t-head"><a href="#"><img src="${staticURL}/style/images/logo_login.png"/></a></td>
					</tr>
					<tr class="tr">
						<td colspan="4" class="t-head" style="width:34px;vertical-align:top;text-align:center;font-family:arial,微软雅黑; font-size:20px; font-weight:bold;">&nbsp;PACKING
							&nbsp;&nbsp;LIST&nbsp;</td>
					</tr>
					<tr class="tr">
						<td class="t-title" colspan="4" style="text-align: left;">Shipping Marks:</td>

					</tr>
					<tr class="tr">
						<td rowspan="2" colspan="2" class="t-content"><textarea name=""  rows="2" cols="50"
					...
					<tr class="tr">
						...
						<td><input type="text" class="oneLine" name="orderShipStringDate"/></td>
					</tr>
					<tr style="height: 258px;">
						<td colspan="4" style="border-bottom: 1px solid #A4B5C2;border-top: 1px solid #A4B5C2;">
							<table id="datagrid"></table>
						</td>
					</tr>
					<tr class="tr">
						<th colspan="2" style="text-align: right;">SHIPPED ON:</th>
						<td colspan="2"><input type="text" class="oneLine" name="orderShipStringDate" style="width: 180px;"/></td>
					</tr>
					<tr class="tr">
						<th colspan="2" style="text-align: right;">SHIPPED BY:</th>
						<td colspan="2"><input type="text" class="oneLine" name="itemNameCn" style="width: 180px;"/></td>
					</tr>
					<tr class="tr">
						<th colspan="2" style="text-align: right;">PO NO.:</th>
						<td colspan="2"><input type="text" class="oneLine" name="orderPoCode" style="width: 180px;"/></td>
					</tr>
				</table>
			</form>
		</div>
	...
	<div id="packingListAddDialog" style="display: none;width: 800px;height: 230px;overFlow-x: hidden;" align="center">
		...
		<div class="part_zoc zoc">
						...
						<div class="oneline">
							...
							<div class="righttext">
								<input name="haierModel" type="text" class="easyui-validatebox" data-options="required:true" missingMessage="请填写��������"  style="width: 155px;"/>						
							</div>
							<div class="itemleft60">客户型号:</div>
							<div class="righttext">
								<input name="customerModel" type="text" class="easyui-validatebox" data-options="required:true" missingMessage="请填写��������"  style="width: 155px;"/>						
							</div>
							<div class="itemleft60">简码:</div>
							<div class="righttext">
								<input name="simpleCode" type="text" class="easyui-validatebox" data-options="required:true" missingMessage="请填写����"  style="width: 155px;"/>						
							</div>
						</div>
						<div class="oneline">
						...
						<div class="itemleft60">订舱号:</div>
							<div class="righttext">
								<input name="bookCode" id="addBookCode" type="text" class="easyui-validatebox"  missingMessage="请填写订舱号" readonly="readonly" style="width: 155px;"/>						
							</div>
