    ...
    function saveDetail(){
        ...
        if(updated.length){
        	...
        	for(var i = 0; i < updated; i++){
        		if(updated[i].price == "" || updated[i].custPrice == "" || updated[i].price == null || updated[i].custPrice == null){
        			priceFlag = false;
        			break;
        		}
        	}
        	if(!priceFlag){
        		$.messager.alert('<s:text name="global.form.prompt" >提示</s:text>','<s:text name="order.confirm.attachPriceInfo">数量或者单价或者报关金额不能为空,请检查！</s:text>','info');
        	}else{
        		$.ajax({
