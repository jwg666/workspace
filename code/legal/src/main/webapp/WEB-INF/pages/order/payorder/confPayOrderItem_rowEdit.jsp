...
<script type="text/javascript" charset="utf-8">
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			url : 'confPayOrderItemAction!datagrid.action',
			title : '付款保障明细，从付款保障的行项目中，扩展出来N次付款的表。并且本身也允许多行，用来支持同一次付款中，多种付款方式组合列表',
			iconCls : 'icon-save',
			pagination : true,
			pagePosition : 'bottom',
			pageSize : 50,
			...
			columns : [ [ 
			   ...
			   {field:'actConfPayItemCode',title:'付款保障编号',align:'center',sortable:false,width : 85,
			  	    editor : {
							type : 'validatebox',
							options : {}
					},
			   ...
			   {field:'actConfPayCode',title:'actConfPayCode',align:'center',sortable:false,width : 85,
			  	    editor : {
							type : 'validatebox',
							options : {}
					},
			   ...
			   {field:'orderNum',title:'订单编号',align:'center',sortable:false,width : 85,
			  	    editor : {
							type : 'validatebox',
							options : {}
					},
			   ...
			   {field:'orderLinecode',title:'行项目号',align:'center',sortable:false,width : 85,
			  	    editor : {
							type : 'validatebox',
							options : {}
					},
			   ...
			   {field:'paymentMethod',title:'付款方式',align:'center',sortable:false,width : 85,
			  	    editor : {
							type : 'validatebox',
							options : {}
					},
			   ...
			   {field:'payCode',title:'payCode',align:'center',sortable:false,width : 85,
			  	    editor : {
							type : 'validatebox',
							options : {}
					},
			   ...
			   {field:'paymentPeriod',title:'付款周期(单位为天)',align:'center',sortable:false,width : 85,
			  	    editor : {
							type : 'validatebox',
			   ...
			   {field:'amount',title:'金额',align:'center',sortable:false,width : 85,
			  	    editor : {
							type : 'validatebox',
							options : {required:true}
					},
			   ...
			   {field:'currency',title:'币种(与数据字典表关联）',align:'center',sortable:false,width : 85,
			  	    editor : {
							type : 'validatebox',
							options : {}
					},
			...
			onAfterEdit : function(rowIndex, rowData, changes) {
				...
				if (inserted.length > 0) {
					url = 'confPayOrderItemAction!add.action';
				}
				if (updated.length > 0) {
					url = 'confPayOrderItemAction!edit.action';
				}
		...
		if (rows.length > 0) {
			$.messager.confirm('请确认', '您要删除当前所选项目？', function(r) {
				if (r) {
					...
					$.ajax({
						url : 'confPayOrderItemAction!delete.action',
						data : {
							ids : ids.join(',')
						},
						dataType : 'json',
