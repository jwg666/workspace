...
var shipDateLimit  = 0;
var confirmFlag = false;//船期是否与订单出运期相差在 shipDateLimit 天以内
<s:if test="shipDateLimit!=null">
	shipDateLimit = ${shipDateLimit};
</s:if>

var datagrid_bookOrderItem;
var orderList = [];
$(function(){
 	//$.messager.alert('提示', 'taskIds:${taskIds},rowId:${rowId}', 'info');
 	//用于控制页面button是否显示  0是现实 1是不显示  查看已办的时候按钮需要隐藏
 	if('${showButtonFlag}'==1){
 		$("#task_good_div").find("input[type='button']").hide();
 		$("#task_good_div").find("input.print").show();
 	}
 	if('${showButtonFlag}'==0){
 		$("#task_good_div").find("input[type='button']").attr("style","display: inline");
	 	//先隐藏 确认按钮  如果船期符合再显示
 	}
 	if('${showButtonFlag}' == 2){
 		$("#task_good_div").find("input[type='button']").hide();
 		$("#backEnterPaperBtn").show();
 		$("#task_good_div").find("input.print").show();
 	}
 		...
 		success : function(response) {
			...
			if(response.orderBookShipDate!=null){
				if(response.orderBookShipDate == bookShipDateInput.datetimebox("getText")){
					...
					if(bookShipDateInput.datetimebox("getText").length > 0){
						var date = moment(moment(response.orderBookShipDate).format("YYYY-MM-DD"));
						var date2  = moment(moment(bookShipDateInput.datetimebox("getText")).format("YYYY-MM-DD"));
						var sub = date.diff(date2, 'days');
						//3天内可以确认 查过不让确认
						if(sub<=shipDateLimit && sub >=-shipDateLimit){
							confirmFlag = true;
						}
					}
				}
			...
			if('${showButtonFlag}'==0){
				$("#enterGoodsForm").find('[name="orderExecremark"]').removeAttr("readonly");
			}
			//日期空间处理
		...
		columns : [ [ 
		    ...
		    { field:'goodsGrossWeight',title:'毛重',align:'center',width:100,
				formatter:function(value,row,index){
					return Number(row.goodsGrossWeight).toFixed(3);
			}},				
			 ...
			 if(data && data.rows){
				 for(var i=0,l=data.rows.length;i<l;i++){
					 ...
					 for(var j=0;j<orderList.length;j++){
						 if(orderList[j]==data.rows[i].orderCode){
							 isExist = true;
						 }
					 }
					 if(!isExist){
...
function confirmGoods(){
	if(confirmFlag){
		$.messager.confirm('请确认', '您要确认当前下货纸？', function(r) {
			if(r){
				$.messager.progress({text:'系统处理中，请稍等...',interval:200});
				$.ajax({
					url : '${dynamicURL}/bookorder/shipPaperAction!check.do',
					data : {
						bookCode:'${bookCode}',
						orderExecremark : $("[name='orderExecremark']").val(),
						flag:'1'
					},  
					dataType : 'json',
					success : function(response) {
		...
		$.messager.alert('提示', '船期与订单出运期相差大于'+shipDateLimit+'天，下货纸不允许确认！', 'info');
	}
}

function modifyGoods(){
	 $.messager.confirm('系统提示', '是否确认退回下货纸？<br> 如果退回，该订舱将重新回到订舱申请。', function(r) {
		if(r){
			$.messager.progress({text:'系统处理中，请稍等...',interval:200});
			$.ajax({
				url : '${dynamicURL}/bookorder/shipPaperAction!check.do',
...
function backEnterPaper(){
	 $.messager.confirm('系统提示', '是否确认退回到下货纸录入？', function(r) {
		if(r){
			$.messager.progress({text:'系统处理中，请稍等...',interval:200});
			$.ajax({
				url : '${dynamicURL}/bookorder/shipPaperAction!check.do',
...
function resetShipDate(){
	var rowId = $("#rowId").val();
    var app = {"appid":"resetShipDate-"+rowId,"folderId":0,"height":400,"icon":"917","isflash":0,"isopenmax":1,"isresize":1,"issetbar":1,"type":"app","url":"","width":900};
    var name = "调整出运期-订舱号："+$("#bookCode").val();
    var url = "${dynamicURL}/orderedit/orderEditAction!goAutoApplyOrderEdit?batchNo="+orderList.join(",");
    app = $.extend(app,{title:name,url:url});
    //app.imgsrc = HROS.CONFIG.downloadImage+app['icon'];
    parent.HROS.window.createTemp(app);
	
}
function closeCur(msg){
	window.parent.HROS.window.close(currentappid);
	customWindow.refreshTask(msg);
}
...
<body>
	...
	<!-- 	            </td> -->
		        ...
		        <table style="width:100%;border:0;cellpadding:0;cellspacing:0;margin-bottom: 2px"  class="table-p1">
				  <tr style="display: none">
				    <td width="14%" class="t-title">Ex Rate:(兑换率) </td>
				    <td width="14%" class="t-title">Prepaid at(预付地点) </td>
				    <td width="14%" class="t-content">
				      <input name="prePaidPlace" type="text" class="easyui-validatebox" data-options="validType:'length[0,255]'"  />
				    </td>
				    <td width="14%" class="t-title">Payable at Destination (到付地点) </td>
				    <td width="14%" class="t-content">
				      <input name="prePay" type="text" class="easyui-validatebox" data-options="validType:'length[0,255]'"  />
				    </td>
				    <td width="14%" class="t-title">Place of Issue(签发地) :</td>
				    <td width="14%" class="t-content">
					        ...
					        <div class="item66 lastitem" >
								<div class="oprationbutt" align="center">
								  <div class="oprationbutt">
									<input type="button" value="退到下货纸录入" id="backEnterPaperBtn" onclick="backEnterPaper()" /> 
									<input type="button" value="重做订舱" onclick="modifyGoods()" />
									<input type="button" value="调整出运期" onclick="resetShipDate()" />  
									<input type="button" value="确 认" onclick="confirmGoods()" /> 
