...
<script type="text/javascript" charset="utf-8">
	...
	$(function() {
		...
		datagrid = $('#datagrid').datagrid({
			url : 'prepareOrderItemAction!datagrid.action',
			title : '备货单明细表列表',
			iconCls : 'icon-save',
			pagination : true,
			pagePosition : 'bottom',
			...
			columns : [ [ 
			   ...
			   {field:'actPrepareOrderItemCode',title:'明细编号',align:'center',sortable:false,width : 85,
			  	    editor : {
							type : 'validatebox',
							options : {}
					},
			   ...
			   {field:'actPrepareOrderCode',title:'备货单号',align:'center',sortable:false,width : 85,
			  	    editor : {
							type : 'validatebox',
							options : {}
					},
			...
			onAfterEdit : function(rowIndex, rowData, changes) {
				...
				if (inserted.length > 0) {
					url = 'prepareOrderItemAction!add.action';
				}
				if (updated.length > 0) {
					url = 'prepareOrderItemAction!edit.action';
				}
		...
		if (rows.length > 0) {
			$.messager.confirm('请确认', '您要删除当前所选项目？', function(r) {
				if (r) {
					...
					$.ajax({
						url : 'prepareOrderItemAction!delete.action',
						data : {
							ids : ids.join(',')
						},
						dataType : 'json',
