...
/* 	$.ajax({
		...
		success : function(response) {
			...
			datagrid = $('#datagrid').datagrid({
				url : 'custOrderAction!checkTaskBybookCode3.do?bookCode='+$('#bookCode').val()+'&mergeCustFlag='+$('#mergeCustFlagId').val(),
				title : '明细信息',
				iconCls : 'icon-save',
	 			pagination : true,
				pagePosition : 'bottom',
...
/* 	if(endCustDate==null||endCustDate==""){
    ...
    if (custNum!=null&&custNum!=''){
    	if(!(/^[0-9a-zA-Z]*$/g.test(custNum))){
    		$.messager.alert('提示','报关单号不符合要求','warring');
    		return ;
    	...
    	$.messager.alert('提示','报关单号不能为空','warring');
        return ;
    }
	if(custDate==null||custDate==""){
		$.messager.alert('提示','报关时间不可为空','');
		return false;
	}
	if(custNum==null||custNum==''){
		$.messager.alert('提示','报关单号不可为空','');
		return false;
	}
	//sassignee1='123';
	...
	$.ajax({
		url : 'custOrderAction!updatebyOrderCodeonShow.do',
		data : {
			...
			<form id="searchForm">
	            ...
	            <div class="oneline">
	                <div class="item25">
	                    ...
	                    <div class="righttext">
	                    	<input type="text"  name="custCompany"/>
	                    </div>
	                </div>
	                <div class="item25 lastitem">
	                ...
	                <div class="item25 lastitem">
	                    ...
	                    <div class="righttext">
	                    	<input type="text" editable="false" id="custDate" data-options="required:true" class="easyui-datebox" name="custDate"/>
	                    </div>
	                </div>
	                <div class="item25">
	                    <div class="itemleft60"><s:text name="order.custorder.endCustDate">结关时间</s:text>：</div>
	                    <div class="righttext">
	                    	<input name="endCustDate" id="endCustDate"  class="easyui-datebox"   editable="false"/>
	                    </div>
	                </div>
	            </div>
	            <div class="oneline">
